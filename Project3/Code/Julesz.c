...
void getHistogram(const double *filter, const int width, const int height, const double lower, const double upper, const int *image, int *response, const int num_bins, double *filtered)
{
	int i;
	int j;
	int k;
	...
	int width_index;
	int height_index;
	double mult = num_bins / (upper - lower);// = (double)num_bins / MAX_INTENSITY;
	...
	{
        index = (int)(mult * (filtered[i] - lower));
        if (index < 0)
            index = 0;
        if (index >= num_bins)
            index = num_bins - 1;
...
void Julesz(const double **filters, const int *width, const int *height, const double *lower, const double *upper, const double *w, const int num_filters, const int num_bins, const double **orig_response, int *synthesized, double *final_response)
{
	time_t start;
	time_t end;
	...
	{	
		...
		getHistogram(filters[i], width[i], height[i], lower[i], upper[i], synthesized, syn_response, num_bins, syn_filtered[i]);
	...
	while (sum_error > 0 && num_iter < 50) // annealing
	{
		...
		for (i = 0; i < num_filters; i++)
			for (j = 0; j < num_bins; j++)
				sum_error = sum_error + abs(diff_response[i][j]) * w[j];
        ...
        printf("%f %f %f \n", diff_response[0][0], diff_response[0][1], diff_response[0][2]);
        printf("%f %f %f \n", orig_response[0][0], orig_response[0][1], orig_response[0][2]);
	*/
					...
					for (l = 0; l < num_filters; l++)
					{
						...
						for (x = 0; x < H; x++)
						{
							...
							for (y = 0; y < W; y++)
							{
                                ...
                                index1 = (int)(mult * (syn_filtered[l][x2*im_width+y2] - lower[l]));
                                if (index1 < 0)
                                    index1 = 0;
                                if (index1 >= num_bins)
                                    index1 = num_bins - 1;
                                index2 = (int)(mult * (syn_filtered[l][x2*im_width+y2] + k * filters[l][x1*W+y1] - lower[l]));
                                if (index2 < 0)
                                    index2 = 0;
                                if (index2 >= num_bins)
                                    index2 = num_bins - 1;
				...
				for (l = 0; l < num_filters; l++)
				{
					...
					for (x = 0; x < H; x++)
					{
						...
						for (y = 0; y < W; y++)
						{
							...
							//change the histograms
                            index2 = (int)(mult * (syn_filtered[l][x2*im_width+y2] - lower[l]));
                            if (index2 < 0)
                                index2 = 0;
                            if (index2 >= num_bins)
                                index2 = num_bins - 1;
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])                
{
    ...
    lower = (double *)mxGetPr(prhs[4]);
	upper = (double *)mxGetPr(prhs[5]);
    w = (double *)mxGetPr(prhs[6]);
    last = (int *)mxGetPr(prhs[7]);
	...
	plhs[0] = mxCreateDoubleMatrix(num_filters, num_bins, mxREAL);

	//synthesized = (int *)mxGetPr(plhs[0]);
	syn_response = mxGetPr(plhs[0]);

	//srand(time(NULL));
	//for (i = 0; i < im_width * im_height; i++)
	//	synthesized[i] = last[i];//rand() % (MAX_INTENSITY + 1);

	Julesz(filters, width, height, lower, upper, w, num_filters, num_bins, orig_response, last, syn_response);
	free(orig_response);
	free(filters);
}
