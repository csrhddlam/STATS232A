...
void getHistogram(const double **filter, const int *width, const int *height, const double *lower, const double *upper, const int num_filters, const int *image, const int im_width, const int im_height, double *response)
{
	int i;
	int j;
	int k;
	...
	for (i = 0; i < num_filters; i++)
	{
		...
		{
            index = (int)(mult * (filtered[j] - lower[i]));
            if (index < 0)
                index = 0;
            if (index >= num_bins)
                index = num_bins - 1;
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])                
{
	...
	getHistogram(filters, width, height, lower, upper, num_filters, image, im_width, im_height, response);
	free(filters);
}